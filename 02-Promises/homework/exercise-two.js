...
function problemA () {
 ...
 var p1= promisifiedReadFile('poem-two/stanza-01.txt')
  .then(function successHandler1(result) {
    console.log('-- A. promise version (stanza one) --');
    blue(result); 
  })
  var p2= promisifiedReadFile('poem-two/stanza-02.txt')
  .then(function successHandler1(result) {
    console.log('-- A. promise version (stanza two) --');
    blue(result); 
    
  })
  Promise.all([p1,p2]).then(()=>console.log('-- A. callback version done --'))
}
...
function problemB () {
  ...
  .then(function successHandler(result) {
    ...
    Promise.all(promesas).then(()=>console.log('-- B. callback version done --'))
...
function problemD () {
  ...
  .then(function successHandler(result) {                                                                                     10
    ...
    Promise.all(promesas).then((res)=>{res.map((r,i)=>{
      if(r.Error==undefined){
      blue(r);
      
    }else{
      magenta(new Error(r.Error));
